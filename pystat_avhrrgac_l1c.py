...
parser.add_argument('-i', '--inpdir', 
	  help='Path, e.g. /path/to/files', required=True)
...
parser.add_argument('-t', '--test', 
	  help='Run test with reduced channel and select list', action="store_true")
...
for idx, fil in enumerate(fil_list):
	...
	#----------------------------------------------------------------
		...
		try:
		    ...
		    if znobs.sum() != gnobs:
			print (" --- FAILED: Input is fishy due to: %s(zonal nobs) != %s (global nobs) " % (int(znobs.sum()), gnobs) )
			print ("        Fil: %s" % fil)
			print ("       Afil: %s" % afil)
			qflag = False

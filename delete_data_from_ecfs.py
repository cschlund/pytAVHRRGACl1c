...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='''{0} deletes data from ECFS archive
    depending on user request.'''.format(os.path.basename(__file__)))

    parser.add_argument('-e', '--ecfs_basepath', type=str, required=True, help="/user/path/to/data")

    parser.add_argument('-p', '--pattern', type=str, help='''Search pattern, i.e. --pattern=tar, --pattern=NOAA7''')

    parser.add_argument('-s','--subdir', type=str, nargs='*', help='''Delete this subdirectory.''')
    ...
    if args.pattern:
        ...
        logger.info('''Please tell me what to do: remove files via --pattern or remove directory via --subdir''')
